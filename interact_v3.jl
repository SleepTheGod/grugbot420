...
println(io, "> These live on the engine's IMAGE scan path (they respond to image-binary input, not text), ")
println(io, "> so they are persisted as a node-type feature but not queried as text below.\n")
println(io, "> **Note on grave nodes:** `what happened to extinct animals` is persisted with ")
println(io, "> `is_grave=true` (a dead/memorial node) — it never fires by design, so it is not queried.\n")
let cur = ""
    for (subject, inp, resp, node, action, conf) in results
        ...
        println(io, "**Input:** `$inp`  ")
        println(io, "**Node:** `$node` · **Action:** `$action` · **Confidence:** `$(round(conf,digits=3))`  ")
        println(io, "**Grug:** $resp\n")
        println(io, "---\n")
    end
end
close(io)
